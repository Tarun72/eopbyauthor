...
import epi.array.ElementsOfProgramming;
...
public class NonuniformRandomNumber extends ElementsOfProgramming {

    public static int nonuniformRandomNumberGeneration(List<Integer> values,
                                                       List<Double> probabilities) {
        List<Double> probabilitiesLimits =  new ArrayList<>();
        // let's make limits with probabilities
        double left = 0;
        for (Double probability : probabilities) {
            left = left + probability;
            probabilitiesLimits.add(left);
        }
        Random r = new Random();
        final double uniform01 = r.nextDouble();
        // Find the index of the interval that uniform01 lies in.
        int it = Collections.binarySearch(probabilitiesLimits, uniform01);
        if (it < 0) {
